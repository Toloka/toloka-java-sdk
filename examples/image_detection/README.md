...
### Wait to complete detection pool
...
Returns `true` if all assignments were accepted
...
````java
private boolean validate(String poolId, String verificationProjectId, int number) {     
    String verificationPoolId = createVerificationPool(verificationProjectId,
                "Pool " + number + ". Road sign verification");
...
public void waitForAllResponses(String poolId) {
    ...
    while (true) {
        ...
        if (validate(poolId, verificationProjectId, iterationNumber)) {
            break;
        }
        iterationNumber++;
    }
